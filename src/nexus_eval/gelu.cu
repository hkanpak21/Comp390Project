/**
 * gelu.cu — Ported from vendor/nexus/cuda/src/gelu.cu
...
void GELUEvaluator::gelu(PhantomCiphertext &x, PhantomCiphertext &res) {
  ...
  ckks->encoder.encode(ckks->init_vec_with_value(-3.5), x.chain_index(), x.scale(), p0);
  ckks->encoder.encode(ckks->init_vec_with_value(3.5), x.chain_index(), x.scale(), p1);
  ckks->encoder.encode(ckks->init_vec_with_value(1.0 / 8.5), x.chain_index(), x.scale(), delta);
  ...
  ckks->encoder.encode(ckks->init_vec_with_value(0.5), b1.chain_index(), b1.scale(), zero_point_five);
  ...
  ckks->evaluator.mod_switch_to_inplace(x_2, x_4.chain_index());
  ...
  ckks->evaluator.mod_switch_to_inplace(x_2, x_8.chain_index());
  ...
  ckks->evaluator.mod_switch_to_inplace(x, a2.chain_index());
  ...
  s1.set_scale(ckks->scale);
  s2.set_scale(ckks->scale);
  ckks->evaluator.mod_switch_to_inplace(s2, s1.chain_index());
