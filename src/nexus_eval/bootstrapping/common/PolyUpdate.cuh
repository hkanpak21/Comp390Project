...
class Polynomial {
 public:
  vector<NTL::RR> coeff;
  long deg;
  vector<NTL::RR> chebcoeff;

  Polynomial();
  Polynomial(long _deg);
  Polynomial(long _deg, vector<NTL::RR> _coeff, string tag);
  Polynomial(long _deg, NTL::RR *_coeff, string tag);

  void get_coeff(vector<NTL::RR> &_coeff);
  void showcoeff();
  void showchebcoeff();
...
// void poly_decompose_integrate(int deg, int type, NTL::RR scale, vector<NTL::RR> &coeff, Tree& tree, ofstream &out, evaltype eval);
/**
...
*/
void poly_decompose_integrate(int deg, int input_type, NTL::RR input_scale, vector<NTL::RR> &coeff, Tree &tree, evaltype eval_type, int output_type, NTL::RR output_scale, ofstream &out_decomp_coeff);
