/**
 * softmax_mgpu_align.cu
 *
 * Lane MGPU-NEXUS-MICRO — multi-GPU NEXUS Softmax micro-benchmark.
 *
 * Mimics NEXUS's softmax_test (vendor/nexus/cuda/src/main.cu, COEFF_MODULI[2])
 * at NEXUS's chosen Softmax parameter set:
 *
 *   logN = 16          → poly_degree = 65,536, slot_count = 32,768
 *   COEFF_MODULI[2]    = {58, 40×16, 58} (18 moduli total)
 *   scale              = 2^40
 *   softmax len        = 128 (NEXUS's 128×128 attention setup)
 *
 * Pattern: data-parallel across N GPUs (each thread owns its own
 * PhantomContext + SoftmaxEvaluator).
 *
 * CLI:
 *   softmax_mgpu_align                       (default --calls 100 --n-gpus 4)
 *   softmax_mgpu_align --calls 100 --n-gpus 4 --len 128
 */
...
Config parse_cli(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (a == "--calls"   && i + 1 < argc) c.calls  = atoi(argv[++i]);
            ...
            printf("Usage: softmax_mgpu_align [--calls N] [--n-gpus G] [--len L]\n");
            exit(0);
        }
    }
...
int main(int argc, char **argv) {
    ...
    printf("════════════════════════════════════════════════════════════\n");
    printf("  MGPU-MICRO Softmax — NEXUS Softmax, logN=16, %d GPUs\n", cfg.n_gpus);
        ...
        threads.emplace_back([&, g, my_calls]() {
            ...
            try {
...

                {
                    ...
                    softmax_evaluator.softmax(warm_in, rtn, cfg.len);
    ...
    printf("\n════════════════════════════════════════════════════════════\n");
    printf("  MGPU-MICRO Softmax HEADLINE (logN=16, %d GPUs, len=%d)\n",
           cfg.n_gpus, cfg.len);
    printf("════════════════════════════════════════════════════════════\n");
    ...
    printf("  NEXUS measured H100:       20 ms (single-GPU)\n");
    printf("  multiNEXUS single-GPU:     ~20 ms (Lane ALIGN-SINGLE)\n");
    printf("  multiNEXUS %d-GPU effective: %.2f ms\n", cfg.n_gpus, effective_per_call_ms);
    printf("  speedup vs single-GPU:     %.2fx\n",
           20.0 / std::max(1.0, effective_per_call_ms));
    printf("  speedup vs NEXUS A100:     %.2fx\n",
           1150.0 / std::max(1.0, effective_per_call_ms));
    printf("════════════════════════════════════════════════════════════\n");
    fflush(stdout);
