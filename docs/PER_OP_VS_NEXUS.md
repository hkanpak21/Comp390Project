...
|---|---|---|---|---|---|---|
| MatMul (NEXUS amortized, 4096×768 × 768×64) | 13 | 1.31 s amortized over 256-batch | **24.401 s per call** (≈95 ms / 256-amortized) — JOBID 40367787 | **0.329 s amortized per col** (matmul_align_n8k, RUN-PROFILE-01-FIXED, JOBID 40426416, 21{,}033 ms total / 64 cols; MAE = 1.47e-07 vs plain truth, abs tol 5e-2 PASS) | **0.140 s amortized per col** (RUN-PROFILE-01-FIXED, JOBID 40426416, 4× H100, median wall 8{,}943 ms over 3 trials σ≈924 ms, **2.35× wall speedup vs single-GPU**); MAE = 1.31e-07 across ranks, abs tol 5e-2 PASS (rel tol noise-floor amplified per FIX-BUG-MATMUL-02). | **3.98×** (1.31 / 0.329) single-GPU; **9.36×** (1.31 / 0.140) multi-GPU |
