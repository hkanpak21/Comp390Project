...
\section*{TODO --- to be filled in when measurements land}

\begin{itemize}
\item \textbf{Argmax at vocab=30{,}522.} NEXUS published 2.48\,s on A100 for this vocabulary size (BERT-vocab). We currently have vocab=8 measurements only (their smaller fixture). The vocab=30{,}522 measurement is queued on MN5 as JOBID 40388582 (script \texttt{scripts/mn5/slurm\_argmax\_v30k.sh}); single-GPU + 4-GPU configurations. Result will be appended here when the job lands.
\end{itemize}
...
\item \textbf{Argmax at vocab=30{,}522} (already queued, see TODO above). Closes the remaining cell in the headline table.
\end{enumerate}
