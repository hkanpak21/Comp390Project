#!/bin/bash
#SBATCH --job-name=nexus-h100-baseline
...
#
# We also run `nvidia-smi -l 5` in background to verify how many GPUs they use.
#
# Per-layer BERT-base op counts (from NEXUS §VI / our reading of the paper):
#   MatMul    : 6 per layer (Q,K,V,O + 2 FFN)
#   SoftMax   : 12 per layer (one per head)
#   LayerNorm : 2 per layer
#   GELU      : 1 per layer
#   Bootstrap : 4 per layer (NEXUS's main contribution)
#   Argmax    : 1 per inference (only at end)
# Layers: 12. Argmax: 1 total.

echo "════════════════════════════════════════════════════════════"
echo "  Vanilla NEXUS — Phase 0 R3 baseline on MN5 H100"
...
# NB: TEST_TARGETS uses "SoftMax" (capital M) but the binary's actual stdout
#     prints "[Softmax]" (lowercase m) — see vendor/nexus/cuda/src/main.cu:382.
#     TIMER_TAG below is the case-correct tag the binary actually emits.
OPS=("MatMul" "Argmax" "SoftMax" "LayerNorm" "GELU")
TIMER_TAGS=("MatMul" "Argmax" "Softmax" "LayerNorm" "GELU")
TRIALS=3

for idx in 0 1 2 3 4; do
  OP_NAME="${OPS[$idx]}"
  TIMER_TAG="${TIMER_TAGS[$idx]}"
  echo ""
  echo "════════════════════════════════════════════════════════════"
  ...
  for trial in $(seq 1 ${TRIALS}); do
    ...
    /usr/bin/time -v ./main ${idx} 2>&1 | grep -E "\[${TIMER_TAG}\]|\[NEXUS-driver\]|Mean Absolute Error|Average Error|Elapsed|Maximum resident|User time|System time" || true
  done
done
