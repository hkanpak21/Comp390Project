...
public:
    ...
    void save(std::ostream &stream) const {
        ...
        size_t total = size_ * coeff_modulus_size_ * poly_modulus_degree_;
        uint64_t *h_data;
        cudaMallocHost(&h_data, total * sizeof(uint64_t));
        cudaMemcpy(h_data, data_.get(), total * sizeof(uint64_t), cudaMemcpyDeviceToHost);
    ...
    void load(std::istream &stream) {
        ...
        data_ = phantom::util::make_cuda_auto_ptr<uint64_t>(total, cudaStreamPerThread);
        cudaMemcpyAsync(data_.get(), h_data, total * sizeof(uint64_t),
