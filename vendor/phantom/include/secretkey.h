...
public:
    ...
    */
    ...
    void save(std::ostream &stream) const {
        stream.write(reinterpret_cast<const char *>(&sk_max_power_), sizeof(uint64_t));
        stream.write(reinterpret_cast<const char *>(&poly_modulus_degree_), sizeof(uint64_t));
        stream.write(reinterpret_cast<const char *>(&coeff_modulus_size_), sizeof(uint64_t));
        stream.write(reinterpret_cast<const char *>(&chain_index_), sizeof(uint64_t));

        size_t total = sk_max_power_ * poly_modulus_degree_ * coeff_modulus_size_;
        uint64_t *h_data;
        cudaMallocHost(&h_data, total * sizeof(uint64_t));
        cudaMemcpy(h_data, secret_key_array_.get(), total * sizeof(uint64_t), cudaMemcpyDeviceToHost);
        stream.write(reinterpret_cast<char *>(h_data), total * sizeof(uint64_t));
        cudaFreeHost(h_data);

        // Also save data_rns_ (coeff_modulus_size_ * poly_modulus_degree_)
        size_t rns_total = coeff_modulus_size_ * poly_modulus_degree_;
        uint64_t *h_rns;
        cudaMallocHost(&h_rns, rns_total * sizeof(uint64_t));
        cudaMemcpy(h_rns, data_rns_.get(), rns_total * sizeof(uint64_t), cudaMemcpyDeviceToHost);
        stream.write(reinterpret_cast<char *>(h_rns), rns_total * sizeof(uint64_t));
        cudaFreeHost(h_rns);
    }

    void load(std::istream &stream) {
        stream.read(reinterpret_cast<char *>(&sk_max_power_), sizeof(uint64_t));
        stream.read(reinterpret_cast<char *>(&poly_modulus_degree_), sizeof(uint64_t));
        stream.read(reinterpret_cast<char *>(&coeff_modulus_size_), sizeof(uint64_t));
        stream.read(reinterpret_cast<char *>(&chain_index_), sizeof(uint64_t));

        size_t total = sk_max_power_ * poly_modulus_degree_ * coeff_modulus_size_;
        uint64_t *h_data;
        cudaMallocHost(&h_data, total * sizeof(uint64_t));
        stream.read(reinterpret_cast<char *>(h_data), total * sizeof(uint64_t));
        secret_key_array_ = phantom::util::make_cuda_auto_ptr<uint64_t>(total, cudaStreamPerThread);
        cudaMemcpyAsync(secret_key_array_.get(), h_data, total * sizeof(uint64_t),
        ...
        uint64_t *h_rns;
        cudaMallocHost(&h_rns, rns_total * sizeof(uint64_t));
        stream.read(reinterpret_cast<char *>(h_rns), rns_total * sizeof(uint64_t));
        data_rns_ = phantom::util::make_cuda_auto_ptr<uint64_t>(rns_total, cudaStreamPerThread);
        cudaMemcpyAsync(data_rns_.get(), h_rns, rns_total * sizeof(uint64_t),
                        cudaMemcpyHostToDevice, cudaStreamPerThread);
